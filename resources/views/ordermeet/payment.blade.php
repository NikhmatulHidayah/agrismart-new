        ...
        <div class="col-md-10 col-lg-8">
            <div class="card payment-card animate__animated animate__fadeInUp">
                <div class="card-header payment-header">
                    <h4 class="mb-0"><i class="bi bi-credit-card"></i> Pembayaran Pertemuan</h4>
                </div>
                <div class="card-body">
                    <h5 class="mb-3" style="font-weight: 700; color: #14532d;">Detail Ahli Tani</h5>
                    <ul class="list-group mb-4 payment-list">
                        <li class="list-group-item"><span class="payment-label">Nama:</span> {{ $expert->name ?? '-' }}</li>
                        <li class="list-group-item"><span class="payment-label">Alumni:</span> {{ $expert->dataAhliTani->alumni ?? '-' }}</li>
                        <li class="list-group-item"><span class="payment-label">Tanggal Pertemuan:</span> {{ $date }}</li>
                        <li class="list-group-item"><span class="payment-label">Topik:</span> {{ $topic }}</li>
                        <li class="list-group-item"><span class="payment-label">Harga Pertemuan:</span> <span class="payment-amount">Rp {{ number_format($amount, 0, ',', '.') }}</span></li>
                    </ul>
                    <form id="formPembayaran" action="{{ route('ordermeet.process_payment') }}" method="POST">
                        @csrf
                        <input type="hidden" name="id_expert" value="{{ $expert->id }}">
                        ...
                        <div class="mb-4">
                            ...
                            <small class="text-muted">Pilih salah satu e-wallet yang tersedia.</small>
                        </div>
